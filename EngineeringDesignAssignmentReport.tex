...
高通滤波器：
\begin{align*}
    b &= [3.040 \times 10^{-4}, -2.432 \times 10^{-3}, 8.513 \times 10^{-3}, \ldots] \\
    a &= [1.000, 3.304, 5.500, 5.642, 3.840, \ldots]
\end{align*}

低通滤波器：
\begin{align*}
    b &= [1.221 \times 10^{-3}, 9.770 \times 10^{-3}, 3.420 \times 10^{-2}, \ldots] \\
    a &= [1.000, -2.183, 2.990, -2.531, 1.500, \ldots]
\end{align*}
...
对两种方法得到的时域信号进行定量比较：
\begin{itemize}
    \item \textbf{均方误差（MSE）}：$5.54 \times 10^{-3}$
    \item \textbf{均方根误差（RMSE）}：$7.26 \times 10^{-2}$
    \item \textbf{最大绝对差}：0.957
    \item \textbf{相关系数}：$-0.028$（低相关）
    \item \textbf{信噪比（SNR）}：$-18.07$ dB
\end{itemize}

\textbf{3. 低相关性的原因分析}

Q3 和 Q4 的解调结果相关性很低（$r = -0.028$），这是\textbf{正常且预期的}，原因如下：

\begin{enumerate}
    \item \textbf{滤波器特性根本不同}
    \begin{itemize}
        \item Q3：8阶 Butterworth 滤波器，有渐变的过渡带，在截止频率处衰减 -3 dB
        \item Q4：理想砖墙滤波器，无过渡带，在截止频率处瞬时从 1 变为 0
    \end{itemize}
...
\textbf{调试与优化：}遇到了许多实际问题，如频率搬移方向错误、滤波器系数计算精度、FFT 结果归一化等。通过仔细分析中间结果、对比理论值、单步调试等方法逐一解决。特别是在 Q4 中，最初 Q3 和 Q4 的相关系数极低让我困惑，但通过深入分析滤波器特性，我认识到这是正常现象，不同的实现方式本就会产生不同的波形。
...
\subsubsection{4. Q3 与 Q4 相关性低的困惑}

\textbf{挑战}：两种方法的解调结果相关系数仅为 -0.028，让我一度怀疑实现是否有误。

\textbf{解决}：深入分析两种方法的差异，认识到 Butterworth 滤波器与理想滤波器的特性根本不同，产生不同波形是正常的。通过查阅资料，了解到 Gibbs 现象、相位失真等因素都会导致波形差异。最终确认两种方法都正确解调，只是实现方式不同。
...
\begin{thebibliography}{99}
    ...
    \bibitem{ref1} 郑君里，应启珩，杨为理. 信号与系统（第三版）[M]. 北京：高等教育出版社，2011
    \bibitem{ref2} Alan V. Oppenheim, Ronald W. Schafer. Discrete-Time Signal Processing (3rd Edition) [M]. Pearson, 2009
    \bibitem{ref3} 程佩青. 数字信号处理教程（第四版）[M]. 北京：清华大学出版社，2013
    \bibitem{ref4} Richard G. Lyons. Understanding Digital Signal Processing (3rd Edition) [M]. Prentice Hall, 2010
    \bibitem{ref5} 高西全，丁玉美. 数字信号处理（第四版）[M]. 西安：西安电子科技大学出版社，2016
    \bibitem{ref6} Steve Klabnik, Carol Nichols. The Rust Programming Language [M]. No Starch Press, 2018
    \bibitem{ref7} RustFFT Documentation. https://docs.rs/rustfft/, 2023
    \bibitem{ref8} Plotters Documentation. https://docs.rs/plotters/, 2023
    \bibitem{ref9} 樊昌信，曹丽娜. 通信原理（第七版）[M]. 北京：国防工业出版社，2012
    \bibitem{ref10} John G. Proakis, Masoud Salehi. Communication Systems Engineering (2nd Edition) [M]. Pearson, 2001
\end{thebibliography}
...
\begin{center}
\texttt{https://github.com/yzy-pro/SignalandSystemBigHomework}
\end{center}
...
\begin{enumerate}
    \item \textbf{模块化设计}：每个功能独立成模块，接口清晰，便于测试和复用
    \item \textbf{类型安全}：利用 Rust 的强类型系统，编译期捕获错误
    \item \textbf{零成本抽象}：高层次抽象不牺牲运行效率
    \item \textbf{内存安全}：所有权系统保证无内存泄漏、无数据竞争
    \item \textbf{错误处理}：使用 \texttt{Result} 和 \texttt{Option} 类型显式处理错误
    \item \textbf{文档完善}：关键函数和算法都有详细注释
    \item \textbf{可扩展性}：易于添加新的滤波器类型、新的分析方法
\end{enumerate}

\textbf{完整代码请访问：} \texttt{https://github.com/yzy-pro/SignalandSystemBigHomework}

\end{document}

\subsubsection{音频读取模块（audio\_reader.rs）}

\begin{lstlisting}[caption=音频文件读取]
use hound;
use std::path::Path;

pub fn read_wav(file_path: &str) -> (Vec<f64>, u32) {
...
\subsubsection{FFT 频谱分析模块（spectrum\_analyzer.rs）}

\begin{lstlisting}[caption=频谱计算与峰值检测]
use rustfft::{FftPlanner, num_complex::Complex};

pub fn compute_spectrum(signal: &[f64]) 
    -> Vec<Complex<f64>> {
...
pub fn find_peak_frequency(
    ...
    let peak_freq = max_idx as f64 * 
        sample_rate as f64 / n as f64;
    (peak_freq, max_mag)
}
\end{lstlisting}

\subsection{Q2：Butterworth 滤波器设计}

\subsubsection{滤波器系数计算（filter\_design.rs）}

\begin{lstlisting}[caption=8阶 Butterworth 滤波器设计]
use std::f64::consts::PI;

pub fn design_butterworth_highpass(
    cutoff_freq: f64,
    sample_rate: f64,
    order: usize
) -> (Vec<f64>, Vec<f64>) {
    // 归一化截止频率
    let wc = 2.0 * PI * cutoff_freq / sample_rate;
    let wc_pre = (wc / 2.0).tan();
    
    // 计算模拟滤波器极点
    let mut poles = Vec::new();
    for k in 0..order {
        let theta = PI * (2.0 * k as f64 + 
            order as f64 + 1.0) / (2.0 * order as f64);
        let real = wc_pre * theta.cos();
        let imag = wc_pre * theta.sin();
        poles.push(Complex::new(real, imag));
    }
    
    // 双线性变换转换为数字滤波器
    let (b_coeffs, a_coeffs) = 
        bilinear_transform(&poles, sample_rate);
...
pub fn frequency_response(
    ...
    for (i, &coeff) in b.iter().enumerate() {
        num += coeff * z.powi(-(i as i32));
    }
    for (i, &coeff) in a.iter().enumerate() {
        den += coeff * z.powi(-(i as i32));
    }
...
\subsection{Q3：时域解调实现}

\subsubsection{IIR 滤波器（iir\_filter.rs）}

\begin{lstlisting}[caption=Direct Form II IIR 滤波器]
pub struct IIRFilter {
    b_coeffs: Vec<f64>,  // 分子系数
    a_coeffs: Vec<f64>,  // 分母系数
    w_state: Vec<f64>,   // 状态变量
}
...
impl IIRFilter {
    ...
    pub fn process(&mut self, input: f64) -> f64 {
        ...
        // 减去反馈项
        for i in 0..self.w_state.len() {
            w -= self.a_coeffs[i + 1] * self.w_state[i];
        }
        
        // 计算输出
        let mut output = self.b_coeffs[0] * w;
        for i in 0..self.w_state.len() {
            output += self.b_coeffs[i + 1] * self.w_state[i];
        }
        
        // 更新状态
        for i in (1..self.w_state.len()).rev() {
            self.w_state[i] = self.w_state[i - 1];
        }
        if !self.w_state.is_empty() {
            self.w_state[0] = w;
        }
...
\subsubsection{解调器（demodulator.rs）}

\begin{lstlisting}[caption=同步解调实现]
pub fn demodulate(
    signal: &[f64],
    carrier_freq: f64,
    sample_rate: f64,
    hp_filter: &mut IIRFilter,
    lp_filter: &mut IIRFilter
) -> Vec<f64> {
    let n = signal.len();
    ...
    for i in 0..n {
        // 1. 高通滤波
        let xh = hp_filter.process(signal[i]);
...
\subsection{Q4：频域解调实现}

\subsubsection{理想滤波器（ideal\_filter.rs）}

\begin{lstlisting}[caption=频域理想滤波器]
use rustfft::num_complex::Complex;

pub fn ideal_highpass(
    ...
    for i in 0..n {
        let freq = if i <= n / 2 {
            ...
            (i as f64 - n as f64) * freq_resolution
        };
        
        // 理想高通：|f| < fc 时置零
        if freq.abs() < cutoff_freq {
            spectrum[i] = Complex::new(0.0, 0.0);
        }
    }
}

pub fn ideal_lowpass(
    spectrum: &mut [Complex<f64>],
    cutoff_freq: f64,
    sample_rate: f64
) {
...
\subsubsection{频率搬移（frequency\_shifter.rs）}

\begin{lstlisting}[caption=频域循环移位实现频率搬移]
use rustfft::num_complex::Complex;
...
pub fn frequency_shift(
    ...
    // 计算频率偏移对应的索引数
    let shift_bins = 
        (shift_freq * n as f64 / sample_rate).round() 
        as usize;
    
    let mut result = vec![Complex::new(0.0, 0.0); n];
    
    // 实现 X_b(f) = 0.5 * [X_h(f-fd) + X_h(f+fd)]
    for i in 0..n {
        // 正向移位（f - fd）
        let idx_minus = (i + shift_bins) % n;
        // 负向移位（f + fd）
        let idx_plus = if i >= shift_bins {
            i - shift_bins
        } else {
            n + i - shift_bins
        };
        ...
        // 两个分量加权求和
        result[i] = (spectrum[idx_minus] + 
            spectrum[idx_plus]) * 0.5;
    }
...
\subsubsection{Q4 主程序流程}

\begin{lstlisting}[caption=频域解调完整流程]
fn main() {
    // 1. 读取参数和音频
    let params = read_q1_results();
    let (signal, sample_rate) = read_wav("input.wav");
    
    // 2. FFT 变换到频域
    let mut spectrum = compute_spectrum(&signal);
    ...
    // 6. IFFT 返回时域
    let mut demodulated = compute_ifft(&spectrum);
    
    // 7. 归一化和增益补偿
    for sample in &mut demodulated {
        *sample = *sample * 2.0 / signal.len() as f64;
    }
    
    // 8. 保存结果
    save_wav("Q4_demodulated.wav", &demodulated);
}
\end{lstlisting}

\subsection{可视化模块}

\begin{lstlisting}[caption=使用 Plotters 绘制频谱图]
...
pub fn plot_spectrum(
    ...
    // 计算幅度谱
    let magnitudes: Vec<_> = spectrum[0..n/2]
        .iter()
        .map(|c| c.norm())
        .collect();
    ...
    chart.draw_series(LineSeries::new(
        magnitudes.iter().enumerate().map(|(i, &mag)| {
            let freq = i as f64 * sample_rate as f64 
                / n as f64;
            (freq, mag)
        }),
        &BLUE,
