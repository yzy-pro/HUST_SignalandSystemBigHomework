...
\begin{enumerate}
    \item \textbf{频谱分析}：通过 FFT 分析错误解调信号 $x(t)$ 的频谱，找出频率偏差 $f_d$ 的值。错误解调后的信号频谱关于 $\pm f_d$ 呈现明显的对称关系。
...
组合图展示了两个滤波器的互补关系：
\begin{itemize}
    \item 高通滤波器（蓝色）在 $f > 3000$ Hz 时通过
    \item 低通滤波器（红色）在 $f < 4000$ Hz 时通过
    \item 两者的过渡带有部分重叠（3000--4000 Hz）
\end{itemize}
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[node distance=1.5cm, auto]
        ...
        \node (hp_filter) [process, below of=read_audio] {高通滤波\\\(x_h(t) = h_h(t) * x(t)\)};
        \node (multiply) [process, below of=hp_filter] {载波相乘\\\(x_b(t) = x_h(t) \cos(2\pi f_d t)\)};
        \node (lp_filter) [process, below of=multiply] {低通滤波\\\(x_l(t) = h_l(t) * x_b(t)\)};
...
\begin{itemize}
    \item 采样频率：$f_s = 22050$ Hz
    \item 高通滤波器：$f_c = 3000$ Hz（8阶 Butterworth）
    \item 低通滤波器：$f_B = 4000$ Hz（8阶 Butterworth）
    \item 载波频率：$f_d = 3000.1823$ Hz
\end{itemize}

\textbf{方案B（错误1）：低通 → 相乘 → 高通}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{codes/Q3/output/Q3_error_case1_spectrum.png}
    \caption{错误方案B的频谱分析（LP → Mult → HP）}
    \label{fig:q3_error_case1}
\end{figure}

\textbf{方案C（错误2）：相乘 → 高通 → 低通}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{codes/Q3/output/Q3_error_case2_spectrum.png}
    \caption{错误方案C的频谱分析（Mult → HP → LP）}
    \label{fig:q3_error_case2}
\end{figure}

\textbf{方案D（错误3）：相乘 → 低通（跳过高通）}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{codes/Q3/output/Q3_error_case3_spectrum.png}
    \caption{错误方案D的频谱分析（Mult → LP，跳过HPF）}
    \label{fig:q3_error_case3}
\end{figure}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\textwidth]{codes/Q3/output/Q3_error_comparison.png}
    \caption{四种解调方案的时域波形对比（前 20 ms）}
    \label{fig:q3_error_comparison}
\end{figure}

\textbf{对比分析：}
\begin{itemize}
    \item \textbf{正确方案（绿色）}：正确
    \item \textbf{错误方案1（红色）}：信号的频谱被搬移到高频，听起来尖锐刺耳
    \item \textbf{错误方案2（蓝色）}：正确信号被丢失，剩下大量噪声，较难辨别语音内容
    \item \textbf{错误方案3（品红）}：被原始信号的低频噪声干扰，但此次信号的低频噪声能量较小，听感尚可
\end{itemize}

\textbf{实验结论：}
\begin{enumerate}
    \item \textbf{滤波器顺序至关重要}：错误的顺序会导致有用信号被滤除或噪声污染严重
    \item \textbf{高通滤波器不可或缺}：跳过高通滤波会导致低频干扰搬移到通带内，严重降低信噪比
    \item \textbf{理论与实验一致}：频谱图验证了理论分析的预测，证明了方案A（HP → Mult → LP）的正确性
\end{enumerate}
...
\begin{itemize}
    \item 高通滤波器（$f_c = 3000$ Hz）引入相位失真 $\phi_{HP}(f)$
...
本次设计中，原始信号存在频率偏差 $f_d = f_c - \tilde{f_c} \approx 3000$ Hz，这导致解调不当时信号频谱整体偏移。通过 Q1 的对称峰值分析，准确确定了频谱对称轴位置，Q3 和 Q4 都能正确补偿这个偏差。这说明\textbf{准确理解频谱特征和正确的参数估计方法是后续处理的基础}。
